 ...
 *
 * Authenticated password change for patients.
...
export async function POST(request: Request) {
  try {
    // 1. Require authenticated session (PATIENT role)
    ...
    console.log(`✅ Password changed for patient ${user.email}`)
    ...
    console.error('Password change error (patient):', error)
    return NextResponse.json(
